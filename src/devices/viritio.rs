use crate::{
    bus::{Device, VirtualDevice}, cpu::Cpu, log_debug, memory::{dram::Sizes, virtual_memory::MemorySize}, trap::Exception
};
...
// const VIRTIO_END: u32 = VIRTIO_BASE + VIRTIO_SIZE;
...
impl Virtio {
    pub fn new_device() -> VirtualDevice {
        VirtualDevice::new(Box::new(Self::new()), VIRTIO_BASE, VIRTIO_SIZE)
    }
    ...
    pub fn initialize(&mut self, binary: Vec<u8>) {
        log_debug!("{:-^80}", " Virtio Disk ");
        self.disk.extend(binary);
        log_debug!("{:-^80}", format!(" Loaded bytes {} ", self.disk.len()));
    }
    ...
    pub fn disk_access(cpu: &mut Cpu) -> Result<(), Exception> {
        ...
        let virtio = cpu
            ...
            .expect("virtio device not found") as *mut Virtio;
        let virtio = unsafe { &mut *virtio };
        virtio.interrupt_status |= 0x1;

        let virtq = virtio.virtqueue();
        ...
        match (desc1.flags & VIRTQ_DESC_F_WRITE) == 0 {
            true => {
                ...
                for i in 0..desc1.len {
                    ...
                    virtio.write_disk(sector * SECTOR_SIZE + i, data);
                }
            }
            false => {
                // Read disk data and write it to memory.
                for i in 0..desc1.len {
                    let data = virtio.read_disk(sector * SECTOR_SIZE + i);
        ...
        cpu.bus_write(
            virtq
                .used_addr
                .wrapping_add(4)
                .wrapping_add((virtio.id % QUEUE_SIZE) * 8),
            head_index,
            Sizes::Word,
        )?;

        virtio.id = virtio.id.wrapping_add(1);
        cpu.bus_write(virtq.used_addr.wrapping_add(2), virtio.id, Sizes::HalfWord)?;
