...
impl Exception {
    ...
    pub fn take_trap(&self, cpu: &mut Cpu) -> Trap {
        ...
        if previous_mode <= Mode::Supervisor && ((cpu.state.read(MEDELEG) >> cause) & 1) == 1 {
            ...
            cpu.set_pc((cpu.state.read(STVEC) & !1) as u32);
            ...
            cpu.set_pc((cpu.state.read(MTVEC) & !1) as u32);
            ...
            cpu.state.write_mstatus(MSTATUS_MPIE, cpu.state.read_mstatus(MSTATUS_MIE));
            ...
            match previous_mode {
                ...
                Mode::Supervisor => cpu.state.write_mstatus(MSTATUS_MPP, Mode::Supervisor as u32),
